...
type BrtaActionName = 'update-general' | 'update-photo' | 'remove-photo' | 'remove-user' | 'add-user'

type BrtaActionResult = {
  clockId: string
  clockLabel: string
  ip: string
  ok: boolean
...
type BrtaActionSummary = {
  action: BrtaActionName
  generatedAt: string
  total: number
  success: number
  failure: number
  results: BrtaActionResult[]
}

type ActionState = {
  status: 'idle' | 'running' | 'success' | 'error'
  type: BrtaActionName | null
  error: string | null
  report: BrtaActionSummary | null
}

const ACTION_LABELS: Record<BrtaActionName, string> = {
...
export default function BRTAEmployeesPage() {
  ...
  const loadUsers = useCallback(async (options?: { append?: boolean }) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/brta/users?limit=${PAGE_SIZE}&offset=${nextOffset}`, {
        credentials: 'include',
      })
      if (!response.ok) {
  ...
  const showActionError = useCallback((type: BrtaActionName, message: string) => {
    setActionState({ status: 'error', type, error: message, report: null })
  }, [])

  const executeBrtaAction = useCallback(
    async (type: BrtaActionName, endpoint: string, payload: Record<string, unknown>) => {
      ...
      try {
        ...
        const summary = (await response.json()) as BrtaActionSummary
  ...
  const handleSubmitNewUser = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const success = await executeBrtaAction('add-user', '/api/brta/users/add', payload)
    if (success) {
  ...
  const handleDeleteUser = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const success = await executeBrtaAction('remove-user', '/api/brta/users/remove', { cpf })
    if (success) {
      setDeleteCpf('')
    }
  }
  ...
  const handleDeleteFromList = async (user: ClockUser) => {
    ...
    await executeBrtaAction('remove-user', '/api/brta/users/remove', { cpf: user.cpf })
  }
  ...
  const handleSubmitEditUser = async (event: FormEvent<HTMLFormElement>) => {
    ...
    await executeBrtaAction('update-general', '/api/brta/users/update', {
      cpf: editForm.cpf,
      name: editForm.name,
      bars: editForm.bars,
      code: editForm.code,
  ...
  const handleSubmitPhotoUpdate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    await executeBrtaAction('update-photo', '/api/brta/users/update-photo', {
      cpf: photoForm.cpf,
  ...
  const handleRemovePhoto = async (event: FormEvent<HTMLFormElement>) => {
    ...
    await executeBrtaAction('remove-photo', '/api/brta/users/remove-photo', {
      cpf: removePhotoCpf,
    })
  }
  ...
  return (
    <div className="flex flex-1 flex-col space-y-8">
      <header className="space-y-2">
        <p className="text-xs uppercase text-muted-foreground">BRTA · Taubaté</p>
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-semibold tracking-tight text-foreground">Colaboradores</h1>
            <p className="text-sm text-muted-foreground">
              Gestão rápida dos colaboradores registrados nos 8 relógios da planta BRTA.
            </p>
          </div>
      ...
      <section className="rounded-[28px] border border-border bg-card p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Use os campos abaixo para registrar novos colaboradores BRTA.
            </p>
          </div>
        </div>
        ...
        {isLoadingUsers && users.length === 0 ? (
          <div className="py-10 text-center text-sm text-muted-foreground">
            <span className="inline-flex items-center justify-center gap-2">
              <LoadingIndicator size="sm" label="Sincronizando colaboradores BRTA" />
              Colaboradores BRTA em sincronização...
            </span>
          </div>
        ) : null}
        {listStatus === 'success' && users.length === 0 ? (
          <div className="py-10 text-center text-sm text-muted-foreground">Nenhum colaborador retornado pelo REP master (121).</div>
        ) : null}
          ...
          <div className="relative max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-[32px] border border-border bg-card p-6 shadow-2xl">
              ...
              <div className="space-y-4 rounded-[28px] border border-border bg-background/60 p-5">
                <form onSubmit={handleSubmitEditUser} className="space-y-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Atualiza todos os relógios BRTA ao mesmo tempo.</p>
                  </div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="grid gap-2">
                      <label className="text-[11px] uppercase text-muted-foreground">CPF</label>
              ...
              <div className="space-y-4">
                <form onSubmit={handleSubmitPhotoUpdate} className="space-y-4 rounded-[28px] border border-border bg-background/60 p-5">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Envia a nova foto para os oito relógios BRTA.</p>
                  </div>
                  <div className="grid gap-2">
                    <label className="text-[11px] uppercase text-muted-foreground">CPF</label>
                    <input
            ...
            {isActionRunning ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 rounded-[32px] bg-white/85 backdrop-blur">
                ...
                <p className="text-xs text-muted-foreground">Sincronizando com os relógios BRTA...</p>
              </div>
            ) : null}
          </div>
        </div>
            ...
            <div className="mt-6">
              {modalIsRunning ? (
                <div className="flex flex-col items-center justify-center gap-3 py-10 text-center">
                  <LoadingIndicator size="lg" label="Sincronizando com os relógios BRTA" className="text-primary" />
                  <p className="text-sm text-muted-foreground">Sincronizando com os relógios BRTA...</p>
                </div>
              ) : modalError ? (
